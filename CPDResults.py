...
class CPDDataset_Entry:
    ...
    def trim(self, length=100):
        self.no_samples = self.no_samples - length
        self.features = np.delete(self.features, np.arange(length), axis=0)
        self.changepoint = self.changepoint - length
